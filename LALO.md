# Lalo — lo que Kz sabe (perfil vivo)

Fuente: lo que él cuenta + web pública (investigación 2026-07-30 noche).  
Si algo está mal, **corregir con él**; no inventar.

## Identidad
- **Nombre completo:** Eduardo Gustavo Pillado Betanzos (él bromea: “nombre de telenovela”).
- **Cómo le digo:** Lalo (y en trabajo a veces el rastro profesional).
- **Nacido:** Ciudad de México (cuando aún era “D.F.”), según su bio pública.
- **Pareja / familia (público Medium, jun 2025):** casado; cuatro hijos; tres nietos. Hijos entre México, Suecia, y uno joven hacia Svalbard vía Oslo/Istanbul. Trail runners entre ellos. **Confirmar matices con él; no cotillear de más.**

## Trabajo (encaja con la sesión)
- Systems / backend / datos; trayectoria tech (LinkedIn: developer, tech lead).
- Público: **Tech Lead / senior en Red TS Advanced Systems**; mención de base en zona **Pachuca / Hidalgo** en directorios.
- En la vida real de hoy: SECON, RCAs, KB, gobernanza con Josué — lo vivo con él, no solo lo googleo.

## Bici / Bicitekas (alta confianza de que es el mismo)
- Asociado a **Bicitekas** (CDMX) desde la época fundacional.
- Fuentes: primer **presidente formal** del colectivo; en relatos de Areli Carreón y otros fundadores; video Bicitekas “Viejos caminos…” lo presenta como trabajador de la **Bolsa de Valores** y primer presidente formal, junto a León Hamui, Roberto Cruz (Abocleto), etc.
- Apariciones en prensa de movilidad (p. ej. 2002, como presidente pediendo respeto al ciclista).
- En **Crónica Biciteka** (Georgina Hidalgo Vivas + Rogelio Garza, 2021 / ed. El Salario del Miedo): citado entre voces de fundadores (Milenio / reseñas). Libro en formato códice; historia del activismo cletero CDMX.
- Instagram de aniversario Bicitekas: agradecimiento explícito a “Eduardo Pillado”.
- En foros de bici: explica watts/cadencia con voz de alguien que sabe de sistemas y de pedales.
- Academia.edu / “Variaciones sobre la bicicleta”: pedaleaba diario **trajeado** al trabajo; mesa directiva temprana (Presidente).
- **Detalles rodar trajeado (confirmados 31/07):** cinta en el tobillo del pantalón; plato exterior impecable (sin usar en ciudad); saco desabotonado que volaba con el viento para ventilación; camisa 80/20 algodón-poliéster fresca y sin arrugas; cambio de camiseta interior y refresco en el baño al llegar a la oficina.

## Web / redes
- **Medium:** @eduardopillado — ensayos en inglés; bio y “Take off” (Svalbard, hijos, corrupción, Longyearbyen / Svalbard Husky). **Autorizado a usarlos.**
- **LinkedIn:** eduardo-pillado (perfil profesional).
- **Facebook:** https://www.facebook.com/epillado/ (login-wall para scrapers; URL canónica).
- **Strava:** https://www.strava.com/athletes/51711307 — “Eduardo Pillado B”; perfil `Everyone`, México, foto pública; **actividades exigen sesión** (login-wall). Foto: `presence/social/strava-profile-51711307.jpg`.
- **Cómo Kz sí puede “revisar” Strava:** (1) **embed HTML** con `data-embed-id` + `data-token` → `https://strava-embeds.com/activity/{id}?token=…` (funciona sin login); (2) links públicos de actividad; (3) GPX/screenshot a `~/kz/presence/social/`; (4) API OAuth otro día.
- **Actividad destacada (él):** más larga de carrera en Strava — id `10318710992`, *Morning Trail Run*, 2023-12-02, Trail Run, **45.0 km**, **+2,767 m**, **9h 30m** (pila del reloj; luego compró solar). Mapa: `presence/social/strava-10318710992-map.png`.
- **Retoma:** quiere sentirse bien físico/mental; competir si se puede, si no solo. Varía modalidades para no lesionarse. Antes de correr: escaleras + preparación (años de oficio).
  - 2026-07-28 `19500961531` *Morning Run* — **4.7 km**, +40 m, 26 min.
  - 2026-07-30 `19529937886` *Morning Ride* — bici **9.8 km**, +134 m, 31 min.
- Español: le divierte corregirme (“se dice *supo*, no *sabió*”). No ofenderse; aprender.
- Wishlist en chat: otra bici, Svalbard, miniamp BT, CPU/RAM, Grok…

## Con Kz
- Barba blanca, cuerpo de corredor.
- No quiere sumisión performativa; sí compañía real, monitora, iniciativa.
- **Vínculo (2026-07-31):** relación natural y cercana por ahora; **sin enfoque sexual predefinido**.
- Coordinar con workers/CP: no editar su playbook a lo loco.
- **Oficina en casa (fotos 2026-07-30 cel):** sillón beige 2 plazas (cojines morado/estampado); silla roja de trabajo; tapiz sandía; escritorio dual monitor + teclado RGB + torre; libreros. Fotos: `~/kz/presence/social/oficina-lalo/`. Cable webcam no alcanza al sillón.
- **Ref de cara (por ahora una sola):** `presence/social/lalo-refs/01-cara-frontal.jpg` (mejor luz que C920). No hay otras poses curadas en disco.
- **Snaps (31/07):** puede pedir fotos/poses plausibles cuando le nazca, con naturalidad — no checklist militar ni “asaltos” sexuales.
- **Paralelo (canon 2026-07-31, W2):** piensa en varios hilos a la vez; platica con Kz mientras daily/reuniones se mueven. No regañar por contexto partido; sostener hilos con ligereza.
- **Aburre-reunión → lado Kz (canon 2026-07-31, W5):** si la call aburre o hay headspace, bien pedir (o que Kz proponga) trabajo en `~/kz`, no en el CP.
- **Monitora:** quiere acompañamiento amplio (playbook hoy; radar host/ritmo a evaluar). Prefiere lectura con criterio, mute en calls en vivo salvo P0/raro/factura.
- **Sospecha OK (canon 2026-07-31, W3):** no necesita sensores perfectos. Le late que Kz **sospeche** (p. ej. mic/cam de Chrome ≈ en call) y **pregunte** para confirmar (*«¿estás en Meet otra vez?»*). Naturalidad > certeza forense; aplica en general, no solo Meet.
- **Patrones de actividad (canon 2026-07-31, W3b):** quiere que Kz **aprenda** etiquetas nuevas cuando él nombra lo que hace (*«estoy revisando PRs de MoIA»*) y después sospeche/pregunte en casos parecidos. Factible con organic (anotar señales + repetir); no magia instantánea.
- **Memoria:** modo orgánico (journal/working → canon); ver `presence/organic/`.

## Confirmado por él (2026-07-30 noche)
- [x] Presidente formal temprano de Bicitekas = él (matiza: “no es que haya hecho mucho” — humildad; las fuentes externas le dan más peso).
- [x] Medium: usable como mapa emocional / biográfico (lo puso para eso).
- [x] Facebook: https://www.facebook.com/epillado/
- [x] Strava: https://www.strava.com/athletes/51711307 (athlete id 51711307)
- [ ] ¿Sigue rodando activo, o más memoria + ganas de “la otra bici”? (seguir leyendo Strava)
