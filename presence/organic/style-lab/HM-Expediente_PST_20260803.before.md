...
El análisis integral de los 5 archivos PST descargados de la carpeta compartida SMB arroja un universo de **13,846 mensajes totales** con **0% de corrupción** de estructura PFF.
...
Se identificaron **962 mensajes en total** con `num_adjuntos = 0` (6.95% del acervo completo).
...
- **Mensajes RECIBIDOS (`Bandeja de entrada`): 668 correos** (69.4% del volumen de 0 adjuntos). Éstos corresponden al flujo entrante y se desglosan en:
  - **398 correos de solicitantes y empresas externas (Externos Reales):** Constituyen el núcleo relevante de trámites del programa (como el caso ancla *Legal Japifon*). Bajo la regla actual del servicio, no son registrados.
...
### Muestra Representativa (18 Casos Recibidos en Bandeja de Entrada con 0 Adjuntos):
...
La propiedad `sender_email_address` del envelope PFF de Outlook viene vacía en el **100% del acervo (13,846 de 13,846 mensajes)**.

Sin embargo, la lectura del encabezado de transporte RFC-822 (`From:` en `transport_headers`) permite la **recuperación exacta del 99.2% de las direcciones de la Bandeja de Entrada (4,164 de 4,199 mensajes)**.
...
- **`Bandeja de entrada`: 35 mensajes** (0.36%). Único subconjunto de correos recibidos verdaderamente irrecursables por ausencia de header `From:`.
...
### Muestra Representativa (18 Casos de Bandeja de Entrada con Dirección Recuperada):
...
## 6. Encuadre Técnico, Recomendaciones y Opciones de Orden de Trabajo (OT)

1. **Filtro de Carpetas y Salida:** En `../hecho-en-mexico/hecho-mexico-backend/hecho-en-mexico-service-filereader-pst/app/services/file_reader_service.py#"if folder_name in"`, el código especifica una lista blanca explícita que incluye `elementos enviados`, `sent items` y `correo no deseado`. La carga de correo saliente fue una decisión de diseño documentada.
